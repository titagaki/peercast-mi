...
> peercast-yt 互換アルゴリズムに切り替え済み。詳細は [§10.1](#101-channelid-生成アルゴリズム-実装済み) を参照。
...
2. SessionID / BroadcastID / ChannelID を生成
3. Channel を生成 (ContentBuffer 初期化)、CLI フラグから ChannelInfo を設定
...
8. エンコーダーが RTMP 接続を確立 → onMetaData から ChannelInfo を更新
9. RTMP データを ContentBuffer に流し始める
```
...
4. ch.CloseAll() → 既存の全 PCPOutputStream に quit 送信、全接続を閉じる
