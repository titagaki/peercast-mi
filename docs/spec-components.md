...
7. ストリームデータ送信ループ
   ...
   - 接続相手から bcst を受信した場合: `forwardBcst()` で転送
     - `ttl` が 0 なら破棄
     - `from` が自分の SessionID なら破棄 (ループ防止)
     - 転送時に `ttl -= 1`、`hops += 1`
     - `dest` が特定ノード宛の場合はそのノードにのみ転送
...
5. キーフレームを起点にストリームデータを連続送信
   - ContFlags に InterFrame (0x02) を含まないパケット (= キーフレーム) が来るまでスキップ
   - 以降は全パケットを順次送信
...
`isLocalhost()` でリモートアドレスを検査し、ループバックアドレス以外からのリクエストには HTTP 403 を返す。
