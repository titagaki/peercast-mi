...
小文字英字2文字＋数字4桁のキーは、検証前に Publish 試行数を制限する。最初の試行から60秒の固定窓で、接続元IPごと10回、ノード全体120回。成功・失敗とも数え、全エンコーダー接続で共有する。窓の終了時またはサーバー再起動時にリセットし、超過時はキーを含めないエラーで拒否する。IPはTCP接続元から取得する。既存メディア送信と他形式のキーは対象外。
