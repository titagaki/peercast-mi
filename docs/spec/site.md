...
管理操作は同一オリジンの `POST /admin/api/1` へJSON-RPCを送る。全メソッドで有効なセッション、許可リストのX ID、設定と一致するOrigin、セッションの `X-CSRF-Token` が必要。未ログイン・期限切れは401、一般利用者・CSRF不一致は403。Content-Typeはapplication/json（それ以外415）、クエリは不可（400）、本文上限は1MiB。通常構成では同一プロセス内のJSON-RPCハンドラーを呼び、検証済み操作者をcontextで渡す。ブラウザーの認証ヘッダーは渡さない。ハンドラー未設定の組込構成では固定のloopbackノードAPI `/api/1` へ中継し、接続失敗は502。利用者指定の転送先は受け付けない。
...
| `trusted_proxies` | 作成元IPの判定で信頼するHTTPプロキシのCIDR配列。既定空。不正CIDRは起動エラー。認証や開発ログインの接続元制限には適用しない |
...
サイト一覧は YP 設定順・掲載順。同じ ID は最初の YP を採用し、ゼロ ID の告知行は除外する。自ノードに同じ ID がある場合は受信状態を反映し、取得済みのローカル情報を優先する。ただし一覧・視聴ページの視聴人数は常に採用した YP の値を使い、自ノードの配信・中継の人数で上書きしない。リレー接続直後で CHAN_INFO 未取得の場合は YP の名前・形式を保持する。自ノードだけにあるチャンネルは追加しない。取得先未設定・取得結果が空の場合もローカル一覧へフォールバックしない。本人の配信ページと管理パネルでは引き続きローカルの配信枠を確認できる。
...
チャンネル一覧は画面幅に応じて最大3列で表示する。カード全体は個別視聴ページへの通常のリンク。名前・アイコン・説明・余白のどこからでも移動でき、Tab / Enter、新しいタブで開く操作にも対応する。カード内にリンクを入れ子にしない。
...
ChannelView の任意フィールド `comment` は配信者コメント（掲示板コメントとは別）、`uptime` は秒単位の配信経過時間。不明な uptime は省略する。YP 由来は YP の値を使い、ローカル中継を開始しても中継経過時間に置き換えない。自ノードの配信は自ノードでの開始からの経過時間を使う。一覧・視聴ページの視聴人数は YP の番組一覧の値で統一し、負数・不明は表示しない。ローカルの人数による補完はしない。

サイトのヘッダーはトップページへのリンク「PecaMI」を表示し、左のアイコンからも同じトップページへ移動できる。サブタイトルは表示しない。HTML の title とフッターのアプリ名は「PecaMI」。favicon はオリジン直下の `/favicon.ico` を参照し、サイトトップと共有する。
...
`/broadcast` へは「配信する」リンクで移動する。本人の配信キー発行・配信枠作成はこのページで行う。配信キーの伏字は実際の文字数に合わせる（6文字のキーは `••••••`）。見出しは「チャンネル作成」。配信先・配信キーはフォームの上に枠付きで配置し、フォームと同じラベル／値の2列（スマホでは縦並び）で表示する。案内は作成後にOBS等へ設定することとキーを他人に教えないことに絞る。フォームはチャンネル名・ジャンル・詳細・コメント・コンタクトURLの順で、掲示板情報・新スレ移動は入力欄と同じ列に揃える。送信ボタンは「作成」。ビットレート入力欄はなく、作成時は `bitrate: 0` を送信する。RTMPメタデータの映像レート（maxBitrate優先、なければvideodatarate）とaudiodatarateから更新する。メタデータにレートがなければ0のまま。コメントとURLはチャンネル情報に反映され、一覧・視聴ページにも返す。フッターはアプリ名と管理パネルリンクを表示する。YP の取得設定に関する状態表示は閲覧画面に出さない。
...
サイトの配信枠作成時に `TrackInfo.Creator` を `接続元IP via PecaMI` として設定する（例: `153.229.89.75 via PecaMI`）。PCPでは `trck` の `crea` となり、YP・下流のトラック情報に含まれる。IPはチャンネルを作成したHTTP要求から取得する。RTMP接続元やフォーム入力ではなく、設定履歴には保存しない。既存配信を遡及更新しない。

通常はソケット接続元IPを使用する。ソケット接続元が `site.trusted_proxies` に含まれる場合は `X-Forwarded-For` を右から左に辿り、最初の非信頼IPを採用する。不正ホップまたは列の末端で探索を止め、その時点で確認できたIPを使用する。IPv4-mapped IPv6はIPv4に正規化する。他の転送ヘッダーは使用しない。ソケット接続元自体を解釈できない要求は400。

プロキシは転送ヘッダーを正しく設定し、信頼CIDR内から第三者が直接サイトHTTPへ接続できない構成で運用する。未設定時は転送ヘッダーを無視する。JSON-RPCで作成する配信のトラック情報は指定値のまま。
