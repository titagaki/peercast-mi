...
> 2026-09-14 再検証: このノートは過去の挙動合わせの経緯を含む。現在の 3 実装の比較は [実装比較レポート](../reviews/2026-09-14-implementation-comparison.md) を参照。特に Overflow の時間基準、退出条件、初期バックログ、YP 更新の説明には、現在のコードとの違いがある。「修正済み」は完全一致を意味しない。
